...
- Perfis: `desenvolvedora`, `administrador` e `operador`. `roles.js` e a fonte atual do RBAC da interface.
...
- Lacuna atual: a navegacao do operador ainda exibe Dashboard, Produtos, PDV e Historico, embora acoes privilegiadas de Produtos sejam ocultadas. Isso nao satisfaz sozinho a restricao permanente ao PDV e deve ser corrigido apenas em uma etapa funcional coordenada com as permissoes do backend.
